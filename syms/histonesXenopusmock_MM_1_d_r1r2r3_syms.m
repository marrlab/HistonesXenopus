function [model] = histonesXenopusmock_MM_1_d_r1r2r3_syms()
...
syms 'b' 'K20m1_0' 'K20m2_0' 'K20m3_0' 'noise' 'r1_m' 'r2_m' 'r3_m' 'd'
% create parameter vector
model.sym.p = [b K20m1_0 K20m2_0 K20m3_0 noise r1_m r2_m r3_m d];
...
model.sym.xdot(1) = -r1_m*K20P+d*K20m1+log(2)/(0.5+(b*t)/(b+t))...
    *(K20P+K20m1+K20m2+K20m3)-(log(2)*(0.5*(b+t)^2+b*t^2)/(0.5*(b+t)+b*t)^2)*K20P;
model.sym.xdot(2) = r1_m*K20P-(r2_m+d)*K20m1+d*K20m2...
    -(log(2)*(0.5*(b+t)^2+b*t^2)/(0.5*(b+t)+b*t)^2)*K20m1;
model.sym.xdot(3) = r2_m*K20m1-(r3_m+d)*K20m2+d*K20m3...
    -(log(2)*(0.5*(b+t)^2+b*t^2)/(0.5*(b+t)+b*t)^2)*K20m2;
model.sym.xdot(4) = r3_m*K20m2-d*K20m3...
    -(log(2)*(0.5*(b+t)^2+b*t^2)/(0.5*(b+t)+b*t)^2)*K20m3;
