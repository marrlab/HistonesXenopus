...
%}
%%
function[h,L,MX,MED,bw]=violin(Y,xcoor,varargin)
if length(find(Y == 0)) == length(Y)
else
    %defaults:
    ...
    for i=i:size(Y,2)
        if isempty(lc) == 1
            ...
            if setX == 0
                h(i)=fill([F(:,i)+xcoor;flipud(xcoor-F(:,i))],[U(:,i);flipud(U(:,i))],fc(i,:),'FaceAlpha',alp,'EdgeColor',lc);
            else
